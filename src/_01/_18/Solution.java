    ...
    public static class ReaderThread extends Thread {
        ...
        public void run() {
            while (!Thread.currentThread().isInterrupted()) {
                try {
                    synchronized (reader){//только один поток может одновременно выполнять код внутри этого блока.
                        if (reader.ready()) {//без этой проверки считывание подвисает и выдаёт ошибку(((
